...
import java.util.Optional;
    public class Controller implements Initializable {
        ...
        private Shape createGameStructuralGrid(){
            Shape rectangleWithHoles = new Rectangle((COLUMNS + 1) * CIRCLE_DIAMETER, (ROWS + 1) *
                    CIRCLE_DIAMETER);
        ...
        private void insertDisc(Disc disc, int column){
            ...
            if (row<0) //If it is full, we cannot insert anymore disc
                return;
            insertedDiscsArray[row][column] = disc; //For Structural Change: For developers
            insertedDiscsPane.getChildren().add(disc); //for players
            disc.setTranslateX(column * (CIRCLE_DIAMETER +5) + CIRCLE_DIAMETER / 4);
            int currentRow = row;
            TranslateTransition translateTransition = new TranslateTransition(Duration.seconds(0.5), disc);
            disc.setTranslateY(row * (CIRCLE_DIAMETER + 5) + CIRCLE_DIAMETER / 4);
        ...
        private boolean gameEnded(int row, int column) {
            ...
            List<Point2D> diagonal1Points = IntStream.rangeClosed(0, 6)
                    ...
                    .collect(Collectors.toList());
            Point2D startPoint2 = new Point2D(column - 3,column + 3);
        ...
        public void resetGame() {
            ...
            for(int row = 0; row < insertedDiscsArray.length; row++){ //structurally make all elements back to
                null
